...
export default function SalesChannelTable() 
{
    ...
    const getID = async (id) => {
        const resp = await PaymentServices.getIdReq(id);
        console.log(resp)
        if (resp.data.success  == true) {
            setDataForm(() => {
                return {
                    ...
                    sat_code: resp.data.data.sat_code
                }
            })
            setModalForm(!modalForm)
        }
    ...
    const sendForm = async (event) => {
        ...
        if (dataform.id == false) {
            let res = await PaymentServices.updateRecord(dataform);
            if (res.status == true) {
            ...
            let res = await PaymentServices.storeRec(dataform);
            if (res.status == true) {
                showNotification("succ")
                setModalForm(!modalForm)
            } else {
    ...
    const deleteClient = async (id) => {
        ...
        const successDelete = async (id) => {
            ...
            const req = await PaymentServices.deleteRecord(id);
        ...
        <>
                ...
                <GridItem xs={12}>
                    <Card>
                        <CardHeader color="warning" icon>
                            ...
                            <h4 className={classest.cardIconTitle}>Lista de Metodos de Pago</h4>
                            <Button
                                className="btn-round"
                                color="info"
                                onClick={() => openModal()}
            ...
            >
                ...
                >
                    ...
                    <h4 className={classes.modalTitle}>Formulario de Metodos de pago {dataform.id}</h4>
                </DialogTitle>
                <DialogContent
                ...
                >
                    <GridContainer>
                        ...
                        <GridItem xs={12} sm={6}>
                            <CustomInput
                                id="satCode"
                                labelText="Codigo del Sat"
                                formControlProps={{
                                    fullWidth: true,
                                }}
                                inputProps={{
                                    type: "text",
                                    defaultValue: dataform.sat_code,
                                    name: "sat_code",
                                    onChange: (event) => {
                                        handleInputChange(event)
                                    },
